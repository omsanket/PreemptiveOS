...
{  
	...
	if(n == 0)
	{
		...
		{
			uint64_t c=(uint64_t)kread(((int)(regs->rdi)),((void*)(regs->rsi)),((uint64_t)(regs->rdx)));
			regs->rax=(uint64_t)(c);
			return c;
		}
	}
	...
	{
		ret=kclosedir((uint64_t)(regs->rdi));
		regs->rax=(uint64_t)(1);
		return ret;
	}
	...
	{
		fd=(int)kopen(((char*)(regs->rdi)),((int)(regs->rsi)));
		regs->rax=(uint64_t)(fd);
		return (uint64_t)fd;
	}		
	else if(n==3)                                                
	{
		ret=kclose(((int)(regs->rdi)));
		regs->rax=(uint64_t)(0);
		return ret;
	}	
	...
	else if(n==8)    //lseek
	{
		...
		ret=(uint64_t)klseek(((int)(regs->rdi)),((uint64_t)(regs->rsi)),((int)(regs->rdx)));
